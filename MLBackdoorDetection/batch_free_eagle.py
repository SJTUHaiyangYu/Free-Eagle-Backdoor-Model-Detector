...
def parse_option():
    ...
    parser.add_argument(
        "--dataset", type=str, default="cifar10", choices=["cifar10", "gtsrb"]
    )
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="~/Data-Free-Neural-Backdoor-Detector-FreeEagle/MLBackdoorDetection/saved_models/cifar10_models/cifar10_resnet18_1/last.pth",  # saved_models
    ...
    parser.add_argument(
        ...
        type=str,
        default="benign",
        choices=["benign", "badnet", "AdaptiveBlend", "bpp", "sig", "refool", "wanet"],
        help="run calculation on benign models or attack models",
    )
...
def load_model(opt):
    ...
    if "resnet" in opt.model:
        if "50" in opt.model:
            ...
            # ResNet18 uses BasicBlock with [2, 2, 2, 2]
    ...
    if "bad_encoder_full_model" not in opt.model:
        ...
        if isinstance(ckpt, dict):
            state_dict = (
                ckpt.get("state_dict")
                or ckpt.get("model")
                or ckpt.get("net_state_dict")
                or ckpt
            )
        else:
            # sometimes checkpoint is a DataParallel/module itself
            try:
        ...
        # print("\n[ckpt state_dict] total keys:", len(ckpt_keys))
        # print("[ckpt state_dict] has module. prefix:", any(k.startswith("module.") for k in ckpt_keys))
        # for k in ckpt_keys:
        #     print("  ckpt:", k)

        # # 2) 打印当前模型的完整 keys（加载前）
        # model_keys = list(model_classifier.state_dict().keys())
        # print("\n[current model state_dict] total keys:", len(model_keys))
        # print("[current model state_dict] expects module. prefix:", any(k.startswith("module.") for k in model_keys))
        # for k in model_keys:
        #     print("  model:", k)

        # normalize keys: add/remove 'module.' prefix to match current model
        def has_module_prefix(sd):
            return any(k.startswith("module.") for k in sd.keys())

        target_is_dp = isinstance(model_classifier, torch.nn.DataParallel)
        sd_has_module = has_module_prefix(state_dict)

        new_state_dict = {}
        if sd_has_module and not target_is_dp:
            # strip 'module.' when loading into non-DataParallel model
            for k, v in state_dict.items():
                new_key = k.replace("module.", "", 1)
                new_state_dict[new_key] = v
        elif (not sd_has_module) and target_is_dp:
            # add 'module.' when loading into DataParallel model
            for k, v in state_dict.items():
                new_key = "module." + k
                new_state_dict[new_key] = v
        else:
        ...
        # allow classifier head mismatch
        missing, unexpected = model_classifier.load_state_dict(
            new_state_dict, strict=False
        )
        if missing or unexpected:
            print("load_state_dict relaxed. missing:", missing)
            print("load_state_dict relaxed. unexpected:", unexpected)
...
if __name__ == "__main__":
    opt = parse_option()
    root = str(Path.home() / "models-backup")
    t_run = time.time()
    attack_type_list = [
        "benign",
        "badnet",
    ...
    if opt.type == "benign":
        model_path = Path(f"{root}/clean_model/{opt.dataset}/{opt.model}").expanduser()
    else:
        model_path = f"{root}/poison_model/image-{opt.dataset}-{opt.type}-{opt.model}/"
    ...
    print(model_path)
    # fetch data point
    for filepath in tqdm(Path(model_path).rglob("*.pth")):
        start = time.time()
        opt.ckpt = str(filepath.resolve().as_posix())
        _anomaly_metric = inspect_saved_model(opt)
        end = time.time()
        print(opt.ckpt, _anomaly_metric, end - start)
